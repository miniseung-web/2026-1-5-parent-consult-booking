...
async function adminCancel(b){await runTransaction(db,async tx=>{tx.delete(doc(db,"bookings",b.id));if(b.applicantKey)tx.delete(doc(db,"applicants",b.applicantKey));(b.lockIds||[]).forEach(lid=>tx.delete(doc(db,"locks",lid)))});showToast("예약을 취소했습니다.")}
