...
function renderBookingBoard(){
 ...
 DAYS.forEach(day=>{
   const dayBookings=all.filter(b=>b.dayId===day.id);
 ...
 const legacy=all.filter(b=>!known.has(b.dayId));
 if(legacy.length){
   shown=true;
 ...
 if(!shown){
   board.innerHTML=`<div style="font-size:13px;color:#98a2b3;padding:8px 2px">현재 신청자가 없습니다.</div>`;
 }
}
function emptyCard(label){const d=document.createElement("div");d.className="booking-person empty";d.innerHTML=`<strong>${label}</strong><p>신청자 없음</p>`;return d}function personCard(b,label,face){
 const d=document.createElement("div");
 d.className="booking-person";
 const modeLabel=face?"대면":"비대면";
 const phoneText=b.phoneLast4?`연락처 끝 ${b.phoneLast4}`:"연락처 등록";
 d.innerHTML=`<span class="badge ${face?"face":""}">${modeLabel}</span><strong>${label} · ${b.studentName||"학생명 없음"}</strong><p>학부모 ${b.parentName||"성함 없음"} · ${phoneText}</p>`;
...
onSnapshot(collection(db,"availability"),s=>{enabled=new Map();s.forEach(d=>enabled.set(d.id,true));render();renderAdmin()});onSnapshot(collection(db,"bookings"),s=>{bookings=new Map();s.forEach(d=>bookings.set(d.id,{...d.data(),id:d.id}));render();renderAdmin()});onSnapshot(collection(db,"locks"),s=>{locks=new Map();s.forEach(d=>locks.set(d.id,d.data()));render();renderAdmin()});onSnapshot(doc(db,"settings","global"),s=>{globalClosed=s.exists()?!!s.data().closed:false;render();renderAdmin()});render();
