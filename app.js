...
async function adminCancel(b){
 try{
   if(!b.id)throw Error("예약 문서 ID를 확인할 수 없습니다.");
   await runTransaction(db,async tx=>{
     tx.delete(doc(db,"bookings",b.id));
     if(b.applicantKey)tx.delete(doc(db,"applicants",b.applicantKey));
     (b.lockIds||[]).forEach(lid=>tx.delete(doc(db,"locks",lid)));
   });
   showToast("예약을 취소했습니다.");
 }catch(err){
   console.error(err);
   showToast(err.message||"관리자 취소 중 오류가 발생했습니다.",true);
 }
}
