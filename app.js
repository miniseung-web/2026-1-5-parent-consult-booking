...
const normal=[["1교시","09:00","09:45"],["2교시","09:55","10:40"],["3교시","10:50","11:35"],["4교시","11:45","12:30"],["5교시","13:20","14:05"],["6교시","14:15","15:00"],["7교시","15:10","15:55"],["방과후","16:20","16:40"]];
...
const normalBase=normal.filter(p=>p[0]!=="방과후");
const normalWithAfter=[...normalBase,["방과후","16:20","16:40"]];
const DAYS=[
 {id:"2026-08-31",label:"8월 31일",weekday:"월요일",short:true,periods:monday,teaching:[0,1,3,5]},
 {id:"2026-09-01",label:"9월 1일",weekday:"화요일",short:false,periods:normalWithAfter,teaching:[3,5,6]},
 {id:"2026-09-02",label:"9월 2일",weekday:"수요일",short:false,periods:normalBase,teaching:[2,4,5]},
 {id:"2026-09-03",label:"9월 3일",weekday:"목요일",short:false,periods:normalWithAfter,teaching:[0,2,3,5,6]},
 {id:"2026-09-04",label:"9월 4일",weekday:"금요일",short:false,periods:normalWithAfter,teaching:[1,2,4,5]}
];
const $=id=>document.getElementById(id);let enabled=new Map(),bookings=new Map(),locks=new Map(),adminNotes=new Map(),selected=null,cancelTarget=null,admin=false,globalClosed=false,closedMessage="현재는 새로운 신청을 받지 않습니다.",adminSearch="";let myIds=new Set(JSON.parse(localStorage.getItem("parentConsultBookingIds")||"[]"));
...
$("adminLoginBtn").onclick=()=>{
 if($("adminPassword").value!==ADMIN_PASSWORD)return showToast("관리자 비밀번호가 올바르지 않습니다.",true);
 admin=true;
 $("adminLoginPanel").hidden=true;
 $("adminPanel").hidden=false;
 $("closedMessageInput").value=closedMessage||"";
 renderAdmin();
};
$("toggleGlobalCloseBtn").onclick=async()=>{
 const next=!globalClosed;
 await setDoc(doc(db,"settings","global"),{closed:next,closedMessage:closedMessage},{merge:true});
 showToast(next?"전체 신청을 마감했습니다.":"신청 마감을 취소했습니다.");
};
$("saveClosedMessageBtn").onclick=async()=>{
 const msg=clean($("closedMessageInput").value);
 closedMessage=msg||"현재는 새로운 신청을 받지 않습니다.";
 await setDoc(doc(db,"settings","global"),{closedMessage},{merge:true});
 showToast("마감 안내 문구를 저장했습니다.");
};
$("adminSearchInput").addEventListener("input",e=>{
 adminSearch=clean(e.target.value).toLowerCase();
 renderBookingBoard();
});
$("excelDownloadBtn").onclick=downloadExcel;

function renderAdmin(){if(!admin)return;$("adminCloseText").textContent=globalClosed?"현재 전체 신청이 마감된 상태입니다.":"현재 신청을 받고 있습니다.";if(document.activeElement!==$("closedMessageInput"))$("closedMessageInput").value=closedMessage||"";$("toggleGlobalCloseBtn").textContent=globalClosed?"마감 취소":"전체 신청 마감";$("toggleGlobalCloseBtn").className="btn "+(globalClosed?"primary":"danger");const ad=$("adminDays");ad.innerHTML="";DAYS.forEach(day=>{const c=document.createElement("section");c.className="admin-day";c.innerHTML=`<div class="admin-day-title"><strong>${day.label} (${day.weekday})</strong><span>${day.short?"단축수업 · 35분 1타임":"일반수업 · 비대면 20분×2 / 대면 교시 전체"}</span></div>`;day.periods.forEach((p,i)=>{const isAfter=p[0]==="방과후",locked=day.teaching.includes(i),r=document.createElement("div");r.className="period-row";r.innerHTML=`<div class="period-name">${p[0]}</div><div class="period-time">${p[1]}~${p[2]}</div><div class="period-slots ${locked?"locked":""}">${locked?"수업 중":(isAfter?`${p[1]}~${p[2]} · 비대면 전용 · 상시 공개`:slots(day,p).map(x=>x.label).join(" / "))}</div>`;const lab=document.createElement("label");lab.className="toggle";const inp=document.createElement("input");inp.type="checkbox";inp.disabled=locked||isAfter;inp.checked=isAfter?true:!!enabled.get(pk(day.id,i));const sp=document.createElement("span");sp.className="slider";lab.append(inp,sp);r.append(lab);if(!locked&&!isAfter)inp.onchange=async()=>{const key=pk(day.id,i),ref=doc(db,"availability",key);if(inp.checked)await setDoc(ref,{dayId:day.id,periodIndex:i,enabled:true});else{if(periodHasAnyBooking(day,p)){inp.checked=true;return showToast("예약이 있는 교시는 먼저 예약을 취소해 주세요.",true)}await deleteDoc(ref)}};c.append(r)});ad.append(c)});renderBookingBoard()}
...
function renderBookingBoard(){
 const board=$("adminBookingBoard");
 board.innerHTML="";
 const all=[...bookings.values()];
 const filtered=adminSearch?all.filter(b=>searchableText(b).includes(adminSearch)):all;
 $("adminSearchCount").textContent=`${filtered.length}건 / 전체 ${all.length}건`;
 let shown=false;

 DAYS.forEach(day=>{
   const dayBookings=filtered.filter(b=>b.dayId===day.id);
   if(!dayBookings.length)return;

   shown=true;
   const dayBox=document.createElement("section");
   dayBox.className="booking-day";
   dayBox.innerHTML=`<h4>${day.label} (${day.weekday}) · ${dayBookings.length}건</h4>`;

   day.periods.forEach((p,i)=>{
     const periodBookings=dayBookings.filter(b=>{
       if(Number.isInteger(b.periodIndex)) return b.periodIndex===i;
       const t=(b.start||String(b.time||"").split("~")[0]||"").trim();
       return t>=p[1] && t<=p[2];
     }).sort((a,b)=>String(a.start||a.time||"").localeCompare(String(b.start||b.time||"")));

     if(!periodBookings.length)return;

     const pb=document.createElement("div");
     pb.className="booking-period";
     pb.innerHTML=`<div class="booking-period-head"><strong>${p[0]}</strong><span>${p[1]}~${p[2]} · ${periodBookings.length}건 신청</span></div>`;

     const sg=document.createElement("div");
     sg.className="booking-slots";

     periodBookings.forEach(b=>{
       const label=b.time || (b.start&&b.end ? `${b.start}~${b.end}` : `${p[1]}~${p[2]}`);
       sg.append(personCard(b,label,b.mode==="inperson"));
     });
 ...
 const legacy=filtered.filter(b=>!known.has(b.dayId));
 if(legacy.length){
   shown=true;
   const box=document.createElement("section");
   box.className="booking-day";
   box.innerHTML=`<h4>기타 / 이전 예약 · ${legacy.length}건</h4>`;
   legacy.forEach(b=>{
     const pb=document.createElement("div");
     pb.className="booking-period";
     const sg=document.createElement("div");
     sg.className="booking-slots";
     sg.append(personCard(b,b.time||"시간 정보 없음",b.mode==="inperson"));
     pb.append(sg);
     box.append(pb);
   });
   board.append(box);
 ...
 if(!shown){
   board.innerHTML=`<div style="font-size:13px;color:#98a2b3;padding:8px 2px">${adminSearch?"검색 결과가 없습니다.":"현재 신청자가 없습니다."}</div>`;
 }
}

function emptyCard(label){
 const d=document.createElement("div");
 d.className="booking-person empty";
 d.innerHTML=`<strong>${label}</strong><p>신청자 없음</p>`;
 return d;
}

function personCard(b,label,face){
 const d=document.createElement("div");
 d.className="booking-person";
 const modeLabel=face?"대면":"비대면";
 ...
 noteLabel.textContent="상담 메모";
 const ta=document.createElement("textarea");
 ta.placeholder="예: 성적 상담 희망, 친구관계 문의";
 ta.value=adminNotes.get(b.id)||"";
 ...
 save.onclick=async()=>{
   ...
   if(text){
     await setDoc(doc(db,"adminNotes",b.id),{bookingId:b.id,note:text,updatedAt:serverTimestamp()},{merge:true});
   }else{
     await deleteDoc(doc(db,"adminNotes",b.id));
   }
   showToast(text?"메모를 저장했습니다.":"메모를 삭제했습니다.");
 };
 noteWrap.append(noteLabel,ta,save);

 const actions=document.createElement("div");
 actions.className="booking-person-actions";
 const x=document.createElement("button");
 x.type="button";
 x.className="admin-cancel";
 x.textContent="관리자 취소";
 x.onclick=async()=>{
   if(!confirm(`${b.studentName||"해당 학생"}의 예약을 취소할까요?`))return;
   await adminCancel(b);
 };
 actions.append(x);

 d.append(noteWrap,actions);
 return d;
}

function downloadExcel(){
 const rows=[...bookings.values()]
   .sort((a,b)=>`${a.dayId||""}_${a.start||a.time||""}`.localeCompare(`${b.dayId||""}_${b.start||b.time||""}`))
   .map(b=>({
     "날짜":b.dayLabel||b.dayId||"",
     "요일":b.weekday||"",
     "교시":b.periodName||"",
     "시간":b.time||((b.start&&b.end)?`${b.start}~${b.end}`:""),
     "상담 방식":b.mode==="inperson"?"대면":"비대면",
     "학생 이름":b.studentName||"",
     "학부모 성함":b.parentName||"",
 ...
 if(!rows.length){
   showToast("다운로드할 신청자가 없습니다.",true);
   return;
 }

 const filename=`학부모_상담_신청목록_${new Date().toISOString().slice(0,10)}.xlsx`;

 if(window.XLSX){
   const ws=window.XLSX.utils.json_to_sheet(rows);
   ws["!cols"]=[{wch:12},{wch:9},{wch:9},{wch:16},{wch:10},{wch:14},{wch:14},{wch:16},{wch:32}];
   const wb=window.XLSX.utils.book_new();
   window.XLSX.utils.book_append_sheet(wb,ws,"신청목록");
   ...
   showToast("신청 목록을 다운로드했습니다.");
 }
}
async function adminCancel(b){
 try{
   if(!b || !b.id) throw Error("예약 문서 ID를 확인할 수 없습니다.");

   // 핵심 예약 문서를 우선 직접 삭제합니다.
   // 구버전 예약은 locks/applicants 정보가 없어도 bookings 문서만 지우면 관리자 목록에서 사라집니다.
   await deleteDoc(doc(db,"bookings",b.id));
   try{ await deleteDoc(doc(db,"adminNotes",b.id)); }catch(e){ console.warn("note cleanup",e); }

   // 신버전 예약의 보조 인덱스는 가능한 범위에서 정리합니다.
   if(b.applicantKey){
     try{ await deleteDoc(doc(db,"applicants",b.applicantKey)); }catch(e){ console.warn("applicant cleanup",e); }
   }

   const cleanupLocks=new Set(b.lockIds||[]);
   for(const [lockId,lockData] of locks.entries()){
     if(lockData && lockData.bookingId===b.id) cleanupLocks.add(lockId);
   }
   for(const lockId of cleanupLocks){
     try{ await deleteDoc(doc(db,"locks",lockId)); }catch(e){ console.warn("lock cleanup",lockId,e); }
   }

   showToast("예약을 취소했습니다.");
 }catch(err){
   console.error("관리자 예약 취소 실패:",err);
   alert("예약 취소에 실패했습니다.\n\n"+(err.message||err));
 }
}
onSnapshot(collection(db,"availability"),s=>{enabled=new Map();s.forEach(d=>enabled.set(d.id,true));render();renderAdmin()});
onSnapshot(collection(db,"bookings"),s=>{bookings=new Map();s.forEach(d=>bookings.set(d.id,{...d.data(),id:d.id}));render();renderAdmin()});
onSnapshot(collection(db,"locks"),s=>{locks=new Map();s.forEach(d=>locks.set(d.id,d.data()));render();renderAdmin()});
onSnapshot(collection(db,"adminNotes"),s=>{adminNotes=new Map();s.forEach(d=>adminNotes.set(d.id,d.data().note||""));renderAdmin()});
onSnapshot(doc(db,"settings","global"),s=>{
 globalClosed=s.exists()?!!s.data().closed:false;
 closedMessage=s.exists()&&s.data().closedMessage?s.data().closedMessage:"현재는 새로운 신청을 받지 않습니다.";
 render();
 renderAdmin();
});
render();
